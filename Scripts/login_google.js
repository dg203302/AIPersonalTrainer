...
document.getElementById("boton_google").addEventListener("click",async () => {
    const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        //options: { redirectTo: `https://aipersonaltr.netlify.app/Templates/Inicio/Verificacion_usuario_n.html` },  ESTO PARA PRODUCCION
        options: { redirectTo: `http://localhost:8888/Templates/Inicio/Verificacion_usuario_n.html`}
    });
...
window.onload = () =>{
    supabase.auth.getSession().then(({data: {session}}) => {
        if (session) {
            ...
            //window.location.href = "https://aipersonaltr.netlify.app/Templates/Inicio/Verificacion_usuario_n.html";  ESTO PARA PRODUCCION
        }
    });
}